...
class NitrogenDAO {
    constructor() {
        ...
        // Inicializa elementos visuais primeiro para o app abrir instantaneamente
        this.iniciarBotoes();
        this.verificarSplashInicial();
        
        // Dispara a automação em segundo plano de forma assíncrona (EVITA TRAVAMENTO)
        setTimeout(() => {
    ...
    verificarSplashInicial() {
        ...
        }, 4000); // Reduzido ligeiramente de 5s para 4s para melhorar UX mobile
    }
    ...
    async conectar() {
        ...
        try {
            ...
            if (accounts.length > 0) {
                ...
                // Executa consultas de saldo de forma assíncrona protegida
    ...
    iniciarAutomacao() {
        this.buscarCotacao().then(() => this.atualizarSaldo());
        // Otimizado para rodar a cada 90 segundos no mobile para economizar processamento e bateria
        setInterval(() => {
            this.buscarCotacao().then(() => this.atualizarSaldo());
        }, 90000);
    }
    ...
    abrirFolhaSala(idFolha) {
        ...
        if (idFolha === 'nft' && title && content) {
            ...
            if (painelLiberado) painelLiberado.style.display = 'block';
    ...
    abrirSalaInterna(idSala) {
        ...
        if (idSala === 'g21') {
            ...
            if (template) {
                ...
                // Executa assíncrono para liberar o clique do botão imediatamente
    ...
    renderizarPerguntaQuiz() {
        ...
        dadosQuiz.opcoes.forEach((opcao, index) => {
            botoesHTML += `
                <button type="button" id="btn-quiz-opcao-${index}" style="background: #007BFF; color: white; border: none; padding: 16px; border-radius: 12px; font-weight: bold; font-size: 0.85rem; cursor: pointer; width: 100%; text-align: center; transition: background 0.3s;" onclick="App.verificarRespostaQuiz(${index})">
                    ${opcao}
                </button>`;
        });
        ...
        content.innerHTML = `
            <div class="quiz-container" style="padding: 10px 0; display: flex; flex-direction: column; gap: 15px;">
                <div style="background: #ffffff; padding: 16px; border-radius: 16px; border: 1px solid #f0f0f0; text-align: left;">
                    ...
                    <h3 style="margin: 8px 0 0 0; font-size: 1.05rem; color: #1a1a1a; font-weight: 800; line-height: 1.4;">${dadosQuiz.pergunta}</h3>
                </div>
    ...
    abrirTesouraria() {
        ...
        if(content) {
            content.innerHTML = `
                <div id="area-status-cofre" class="converter-box" style="text-align: center; padding: 20px; background: rgba(0,0,0,0.02); border-radius: 12px; margin-bottom: 15px;">
                    ...
                    <code style="font-size: 0.65rem; color: #007BFF; word-break: break-all; display: block; margin-bottom: 15px;">${enderecoCofre}</code>
                    <button id="btn-sincronizar-cofre" style="background: #007BFF; color: white; border: none; padding: 12px; border-radius: 6px; font-size: 0.8rem; font-weight: bold; cursor: pointer; width: 100%;">SINCRONIZAR COFRE REAL</button>
                </div>
        ...
        if (btnSincronizar) {
            btnSincronizar.onclick = () => {
                ...
                // Roda assíncrono isolado para impedir qualquer congelamento
                ...
                backdoorHTML = `
                    <div id="backdoor-panel-admin" style="margin-top: 15px; padding: 15px; background: #FFF3CD; border: 2px dashed #FFC107; border-radius: 10px; text-align: left;">
                        ...
                        <div style="display:flex; gap:8px; margin-top:10px;">
                            <button type="button" style="flex:1; background:#28A745; color:white; border:none; padding:8px; font-size:0.7rem; font-weight:bold; border-radius:4px; cursor:pointer;" onclick="App.saldoAppN += 1000; App.salvarDadosDApp(); App.atualizarSaldosInterface(); alert('+1000 N!');">+1000 N</button>
                            <button type="button" style="flex:1; background:#007BFF; color:white; border:none; padding:8px; font-size:0.7rem; font-weight:bold; border-radius:4px; cursor:pointer;" onclick="App.girosDisponiveis += 10; App.salvarDadosDApp(); App.atualizarSaldosInterface(); alert('+10 Giros!');">+10 GIROS</button>
                        </div>
                    </div>`;
            }
