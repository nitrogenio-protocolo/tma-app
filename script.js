...
// ==========================================================================
// CONTROLE DOS PAINÉIS DA POUPANÇA E CHECK-IN (PROTOCOLO NITROGÊNIO) - CORRIGIDO
// ==========================================================================

// Criamos uma função global que pode ser chamada diretamente pelo clique do botão HTML
window.App.abrirFolhaPoupancaCustom = function() {
    ...
    panelContent.innerHTML = `
        ...
        <button class="btn-confirm" style="width: 100%;" onclick="App.enviarParaPoupanca()">CONFIRMAR ALOCAÇÃO</button>
    `;
    sidePanel.classList.add("active");
};

window.App.abrirFolhaCheckinCustom = function() {
    const sidePanel = document.getElementById("side-panel");
    const panelTitle = document.getElementById("panel-title");
    const panelContent = document.getElementById("panel-content");
    ...
    panelContent.innerHTML = `
        ...
        <button class="btn-confirm" style="width: 100%; background-color: #28A745;" onclick="App.executarCheckInDiario()">REIVINDICAR ACESSO</button>
    `;
    sidePanel.classList.add("active");
};

// Vincula os cliques diretamente monitorando o ciclo dinâmico do Perfil
document.addEventListener("click", function(evento) {
    if (evento.target && evento.target.id === "btn-checkin-diario") {
        window.App.abrirFolhaCheckinCustom();
    }
    // Caso clique no botão que você gerou dinamicamente para a poupança
    if (evento.target && evento.target.getAttribute("onclick") === "App.enviarParaPoupanca()") {
        // Intercepta e abre a nossa folha customizada com a tabela de níveis educativos
        evento.preventDefault();
        evento.stopPropagation();
        window.App.abrirFolhaPoupancaCustom();
    }
});
