...
class NitrogenDAO {
    constructor() {
        ...
        // Índice e Banco de dados do Quiz Weekly
        this.perguntaAtualIndex = 0;
        this.perguntasQuiz = [
            {
                pergunta: "O que é uma DAO?",
                opcoes: ["Uma Organização Autônoma Decentralizada", "Um tipo de Smart Contract trancado", "O token nativo da rede BSC"],
                correta: 0
            },
            {
                pergunta: "Qual a função do Fundo da Comunidade no Protocolo?",
                opcoes: ["Pagar taxas de gás internas", "Financiar expansões e recompensas dos membros", "Ser queimado mensalmente"],
                correta: 1
            },
            {
                pergunta: "O que significa o termo On-chain?",
                opcoes: ["Dados salvos fora da blockchain", "Transações validadas localmente no aparelho", "Ações registradas e validadas diretamente na Blockchain"],
                correta: 2
            }
        ];
        
        // Recuperação dos dados locais (LocalStorage)
        this.carregarDadosLocais();

        // Inicializa elementos visuais imediatamente para otimizar UX
        this.iniciarBotoes();
        this.verificarSplashInicial();
        
        // Dispara a automação em segundo plano sem travar a Main Thread
        setTimeout(() => {
    ...
    usuarioPossuiNFT() {
        return !!(typeof DEV_MODE !== 'undefined' && DEV_MODE.forcarPossuiNFT);
    }
    ...
    async conectar() {
        ...
        try {
            ...
            if (accounts.length > 0) {
                ...
                this.buscarCotacao().then(() => this.atualizarSaldo());
            }
        } catch (e) { 
    ...
    async buscarCotacao() {
        try {
            ...
            console.error("Falha na cotação secundária.");
        }
    }
    ...
    abrirFolha(tipo) {
        ...
        content.innerHTML = ''; // Limpa estado anterior

        // Injeta a estrutura clonando o template contido exclusivamente no HTML
        const template = document.getElementById(`template-folha-${tipo}`);
        if (!template) {
            console.error(`Template template-folha-${tipo} não foi encontrado no HTML.`);
            return;
        }
        content.appendChild(template.content.cloneNode(true));

        if (tipo === 'receber') {
            if (title) title.innerText = "GERAR COBRANÇA";
    ...
    prepararPagamento(addr, valor) {
        ...
        const template = document.getElementById('template-preparar-pagamento');
        if(!template) return;
        ...
        document.getElementById('pagamento-destino-reduzido').innerText = `DESTINO: ${addr.substring(0,10)}...${addr.substring(addr.length - 4)}`;
        document.getElementById('pagamento-valor-convertido').innerText = valorEmBrl;
    ...
    abrirFolhaSala(idFolha) {
        ...
        // Renderização limpa via templates para evitar injeção massiva de strings
        if (idFolha === 'recompensas') {
            const content = document.querySelector('#sheet-recompensas .panel-content');
            const template = document.getElementById('template-sala-recompensas');
            if (content && template) {
                content.innerHTML = '';
                content.appendChild(template.content.cloneNode(true));
                
                // Mapeamento dinâmico dos escutadores (Removendo onclick inline do HTML)
                document.getElementById('btn-enviar-codigo-comunidade').onclick = () => this.validarCodigoComunidade();
                document.querySelectorAll('.btn-perfil-menu').forEach(botao => {
                    botao.onclick = (e) => {
                        const modulo = e.currentTarget.getAttribute('data-modulo');
                        this.abrirSubModulo(modulo);
                    };
                });
            }
            return;
        ...
        if (idFolha === 'nft') {
            const content = document.querySelector('#sheet-nft .panel-content');
            const template = document.getElementById('template-sala-nft');
            if (content && template) {
                content.innerHTML = '';
                content.appendChild(template.content.cloneNode(true));
            }
            ...
            if (painelLiberated) painelLiberado.style.display = 'block';
        }
    }
    ...
    abrirSalaInterna(idSala) {
        ...
        const templateMap = { 'g21': 'sala-g21', 'teso': 'sala-teso', 'comunidade': 'sala-comunidade', 'recom': 'sala-recom' };
        const templateId = templateMap[idSala];
        const template = document.getElementById(templateId);

        if (template) {
            container.appendChild(template.content.cloneNode(true));
            
            // Caso seja a tesouraria, vincula o comportamento sem conflito
            if (idSala === 'teso') {
                const enderecoCofre = "0x11aBd1b9c71f97ad1df8A0Dbb789f8A96B458219";
                document.getElementById('btn-sincronizar-cofre').onclick = (e) => {
                    e.target.innerText = "CONECTANDO NA BLOCKCHAIN...";
                    e.target.disabled = true;
                    e.target.style.background = "#666";
                    this.executarSincronizacaoReal(enderecoCofre);
                };
                // Dispara sincronia On-chain em segundo plano inicial
                setTimeout(() => this.executarSincronizacaoReal(enderecoCofre), 50);
            }
        }
    }
    ...
    fecharFolhaSala(idFolha) {
        ...
        if (painelAbas) painelAbas.classList.remove('active');
        
        const painelLateral = document.getElementById('side-panel');
        if (painelLateral) painelLateral.classList.remove('active');
    ...
    abrirSubModulo(modulo) {
        ...
        content.innerHTML = ''; // Limpeza de escopo
        ...
        const templateMap = { 'poupanca': 'tela-poupanca', 'checkin': 'tela-checkin', 'roleta': 'tela-roleta' };
        const titleMap = { 'poupanca': "POUPANÇA NITROGEN", 'checkin': "CHECK-IN DIÁRIO", 'roleta': "ROLETA DO BEM" };

        if (title) title.innerText = titleMap[modulo];
        
        const temp = document.getElementById(templateMap[modulo]);
        if (temp) {
            content.appendChild(temp.content.cloneNode(true));
            
            // Vinculação e escuta limpa de eventos e layouts pós-injeção
            if (modulo === 'poupanca') {
                this.atualizarLayoutPoupança();
                document.getElementById('btn-travar-poupanca').onclick = () => this.aplicarPoupança();
                document.getElementById('btn-resgatar-poupanca').onclick = () => this.resgatarPoupança();
            }
            if (modulo === 'checkin') {
                this.atualizarLayoutCheckIn();
                document.getElementById('btn-executar-checkin').onclick = () => this.executarCheckIn();
            }
            if (modulo === 'roleta') {
                document.getElementById('btn-girar-roleta-trigger').onclick = () => this.girarRoleta();
            }
            panel.classList.add('active');
        }
    }
    ...
    renderizarPerguntaQuiz() {
        ...
        content.innerHTML = '';
        const template = document.getElementById('template-quiz-estrutura');
        if(!template) return;

        content.appendChild(template.content.cloneNode(true));

        const dadosQuiz = this.perguntasQuiz[this.perguntaAtualIndex];
        document.getElementById('quiz-contagem-passos').innerText = `Pergunta ${this.perguntaAtualIndex + 1} de ${this.perguntasQuiz.length}`;
        document.getElementById('quiz-texto-pergunta').innerText = dadosQuiz.pergunta;

        const listaOpcoes = document.getElementById('quiz-opcoes-lista');

        dadosQuiz.opcoes.forEach((opcao, index) => {
            const botao = document.createElement('button');
            botao.type = 'button';
            botao.id = `btn-quiz-opcao-${index}`;
            botao.className = 'btn-opcao-quiz-design'; // Gerencie o estilo no seu CSS
            botao.style.cssText = "background: #007BFF; color: white; border: none; padding: 16px; border-radius: 12px; font-weight: bold; font-size: 0.85rem; cursor: pointer; width: 100%; text-align: center; transition: background 0.3s;";
            botao.innerText = opcao;
            botao.onclick = () => this.verificarRespostaQuiz(index);
    ...
    executarCheckIn() {
        ...
        this.checkinDiasConsecutivos = isNaN(this.checkinDiasConsecutivos) ? 0 : this.checkinDiasConsecutivos;
    ...
    resgatarPresenteSurpresa() {
        const hojeString = new Date().toISOString().split('T')[0];
        if(localStorage.getItem('nitrogenio_checkin_surpresa_coletado') === hojeString) return alert("Você já resgatou o prêmio surpresa desse ciclo!");
    ...
    girarRoleta() {
        ...
        if(this.girosDisponiveis <= 0) return alert("Você não possui Giros Disponíveis.");
    ...
    abrirTesouraria() {
        // Redireciona para o fluxo unificado usando as abas estruturadas
        this.mudarAba('governanca');
        this.abrirFolhaSala('governanca');
        this.abrirSalaInterna('teso');
    }
        ...
        try {
            ...
            if (this.provider) {
                ...
                try {
                    ...
                    console.warn("Contrato USDT fora de alcance local.");
                }
                ...
                backdoorHTML = `
                    <div id="backdoor-panel-admin" style="margin-top: 15px; padding: 15px; background: #FFF3CD; border: 2px dashed #FFC107; border-radius: 10px; text-align: left;">
                        ...
                        <div style="display:flex; gap:8px; margin-top:10px;">
                            <button type="button" id="btn-backdoor-add-tokens" style="flex:1; background:#28A745; color:white; border:none; padding:8px; font-size:0.7rem; font-weight:bold; border-radius:4px; cursor:pointer;">+1000 N</button>
                            <button type="button" id="btn-backdoor-add-giros" style="flex:1; background:#007BFF; color:white; border:none; padding:8px; font-size:0.7rem; font-weight:bold; border-radius:4px; cursor:pointer;">+10 GIROS</button>
                        </div>
                    </div>`;
            }
            ...
            if(containerDados) {
                containerDados.innerHTML = `
                    <div class="card-metricas-dao" style="background: rgba(0,0,0,0.02); border: 1px solid rgba(0,0,0,0.05); padding: 15px; border-radius: 10px; text-align: left;">
                        <h3 style="font-size: 0.85rem; color: #333; margin: 0 0 12px 0; font-weight: bold; letter-spacing: 0.5px; border-bottom: 1px dashed rgba(0,0,0,0.1); padding-bottom: 6px;">DIVISÃO DE FLUXO DO PROTOCOLO</h3>
                ...
                // Assina escuta se o painel administrativo backdoor for inserido
                if(this.account && this.account.toLowerCase() === ADDR_ADMIN_BOSS) {
                    document.getElementById('btn-backdoor-add-tokens').onclick = () => {
                        this.saldoAppN += 1000; this.salvarDadosDApp(); this.atualizarSaldosInterface(); alert('+1000 N!');
                    };
                    document.getElementById('btn-backdoor-add-giros').onclick = () => {
                        this.girosDisponiveis += 10; this.salvarDadosDApp(); this.atualizarSaldosInterface(); alert('+10 Giros!');
                    };
                }
            }
        } catch (error) {
    ...
    iniciarBotoes() {
        const btns = { 'btn-pagar': 'pagar', 'btn-receber': 'receber', 'btn-coletar': 'coletar' };
        for (let id in btns) {
        ...
        abasMenu.forEach(aba => {
            ...
            if (el) {
                el.onclick = () => {
                    this.tocarSomClick();
                    this.mudarAba(aba);
                    if(['nft', 'governanca', 'recompensas', 'perfil'].includes(aba)) {
...
function abrirSubFolha(idFolha) {
    ...
    if (folha) folha.classList.add('active');
}

function fecharSubFolha(idFolha) {
    const folha = document.getElementById(idFolha);
    if (folha) folha.classList.remove('active');
}
